...
const TeacherEvents = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`https:backendfinal-production-2920.up.railway.app/getbyteacheridsection/${teacherID}`);
                ...
                if (responseData.length > 0) {
                    ...
                    const eventsPromises = sectionIds.map(async (sectionId) => {
                        const eventsResponse = await axios.get(`https:backendfinal-production-2920.up.railway.app/sectionevents/${sectionId}`);
...
const TeacherAttendance = ({ eventID, teacherID, closeModal }) => {
    ...
    const fetchSections = () => {
        axios.get(`https:backendfinal-production-2920.up.railway.app/sectionevent/${eventID}`)
            .then(response => {
    ...
    const getByTeacherID = () => {
        axios.get(`https:backendfinal-production-2920.up.railway.app/getbyteacheridsection/${teacherID}`)
            .then(response => {
    ...
    const fetchStudents = (sectionID) => {
        axios.get(`https:backendfinal-production-2920.up.railway.app/students/section/${sectionID}`)
            .then(response => {
                const studentsData = response.data;
                const promises = studentsData.map(student => {
                    return axios.get(`https:backendfinal-production-2920.up.railway.app/getstatus/${student.userid}/${eventID}`)
                        .then(statusResponse => {
